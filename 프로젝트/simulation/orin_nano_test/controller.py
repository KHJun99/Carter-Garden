...
class FollowerManager:
    def __init__(self, logger, use_sim_time=True):
        self.logger = logger
    ...
    def start_once(self):
        ...
        self.logger.info(f">>> Loading Follower System into Memory (use_sim_time={self.use_sim_time})...")
        sim_time_str = "true" if self.use_sim_time else "false"
        ros_args = ['--ros-args', '-p', f'use_sim_time:={sim_time_str}']
...
class RobotController(Node):
    def __init__(self):
        ...
        use_sim_time_param = self.get_parameter('use_sim_time').get_parameter_value().bool_value
        self.follower_manager = FollowerManager(self.get_logger(), use_sim_time=use_sim_time_param)
    ...
    def init_callback(self, msg):
        try:
            ...
            p.pose.pose.orientation.z = -0.707
            p.pose.pose.orientation.w = 0.707
            
            # [개선] Rviz 표준 공분산 값 적용 (x:0.25, y:0.25, yaw:0.06)
            cov = [0.0] * 36
            cov[0] = 0.25   # x
            cov[7] = 0.25   # y
            cov[35] = 0.06  # yaw
            ...
            self.get_logger().info(f"초기 위치 전송 완료 (AMCL 초기화): ({data['x']}, {data['y']})")
        except Exception as e:
