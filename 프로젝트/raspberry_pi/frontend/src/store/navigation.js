...
export const useNavigationStore = defineStore('navigation', {
  state: () => ({
    ...
    robotCurrentX: null,
    robotCurrentY: null,
    lastStartPose: { code: 'STOR-001', x: 1.400, y: 1.46 },
  ...
  actions: {
    ...
    getNavigationPayload(startCode, endCode) {
      ...
      const fixedStartPose = actualStartCode === 'STOR-001'
        ? { code: 'STOR-001', x: 1.400, y: 1.46 }
        : actualStartCode === 'GATE-002'
          ? { code: 'GATE-002', x: 1.650, y: -1.56 }
          : null;
      const startNode = this.graph.nodes?.[actualStartCode];
      if (fixedStartPose) {
        this.lastStartPose = fixedStartPose;
      } else if (startNode) {
        this.lastStartPose = {
          code: actualStartCode,
          x: startNode.x,
          y: startNode.y
        };
      } else {
        this.lastStartPose = { code: actualStartCode, x: 1.400, y: 1.46 };
      }
